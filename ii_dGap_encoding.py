...
# ask user for the type of encoding
print("Choose encoding type:\n")
print(" 1. Unary Encoding")
print(" 2. Elias Gamma Encoding")
print(" 3. Elias Delta Encoding")
encoding_type = int(input("Enter your choice: "))
        ...
        if (word in docs[i]):
            ...
            if (encoding_type == 1):
                for i in range(0, len(indices)):    
                    indices[i] = unary_encoding(indices[i])
            elif encoding_type == 2:
                for i in range(0, len(indices)):    
                    indices[i] = elias_gamma_encoding(indices[i])
            elif encoding_type == 3:
                for i in range(0, len(indices)):    
                    indices[i] = elias_detla_encoding(indices[i])
            doc_id = i
